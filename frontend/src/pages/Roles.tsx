...
export default function Roles() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setShowForm(false);
      setEditingRole(null);
      setFormData({ name: '', code: '', description: '', isActive: true, permissionIds: [] });
      loadRoles();
    } catch (err: any) {
      const errorMessage = err.response?.data?.message || err.response?.data?.error || 'Failed to save role';
  ...
  const handleEdit = async (role: Role) => {
    const roleDetails = await loadRoleDetails(role.id);
    if (roleDetails) {
      setEditingRole(role);
      setFormData({
        name: role.name,
        ...
        permissionIds: roleDetails.permissions?.map((p: Permission) => p.id) || [],
      });
    } else {
      setEditingRole(role);
      setFormData({
      ...
      {showForm && (
        <div className="fixed inset-0 z-50 overflow-y-auto">
          <div className="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
            ...
            <div className="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-4xl w-full max-h-[90vh] overflow-y-auto">
                      ...
                      {categories.map(category => (
                        <div key={category} className="mb-6 last:mb-0">
                          <h4 className="text-sm font-semibold text-gray-900 mb-3 flex items-center">
                            <KeyIcon className="h-4 w-4 mr-2 text-primary-600" />
                            {category}
                          </h4>
                          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                            {permissionsByCategory[category]?.map(permission => (
                              <label
                                key={permission.id}
                                className="flex items-start p-2 border-2 rounded-lg cursor-pointer transition-colors hover:bg-gray-50"
                                style={{
                                  borderColor: formData.permissionIds.includes(permission.id) ? '#0284c7' : '#e5e7eb',
                                  backgroundColor: formData.permissionIds.includes(permission.id) ? '#f0f9ff' : 'white',
                                }}
                              >
                                <input
                                  type="checkbox"
